...
**Reason**: The website no longer uses browser-native HTTP Basic Auth. It is replaced by application-managed session login (see the `session-login` capability) plus the tiered access model defined below, which is required to support anonymous read access, in-app logout, password/username changes, and roles.
**Migration**: Existing config-file credentials are no longer used for the website. Users now live in the `users` table; a random-password `admin` is seeded on first startup (see `user-accounts`). Set `auth.enabled: true` to require login.

### Requirement: Users field optional when auth disabled
**Reason**: Website credentials are no longer sourced from `config.yml`. The `auth.users` array is deprecated; user accounts live in the `users` database table.
**Migration**: Remove reliance on `config.yml` `auth.users`. The `users` field is now ignored (kept optional in the schema only for backward-compatible parsing). Manage accounts via the admin user-management API.

### Requirement: Basic Auth excludes External API
**Reason**: Folded into the new "Authorization tiers for website API" requirement, which states that `/external-api/*` uses Bearer Token auth and is not subject to website session auth.
**Migration**: No behavioral change for External API clients; see "Authorization tiers for website API" and "Bearer Token auth for External API".
