...
- **THEN** the system displays a confirmation dialog explaining that this will merge the two tags (irreversible)
- **AND WHEN** user confirms the merge
- **THEN** the system merges all paper associations from the source tag into the target tag and deletes the source tag
...
- **THEN** the tag name reverts to its original value
