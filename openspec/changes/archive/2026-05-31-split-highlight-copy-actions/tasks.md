...
- [x] 1.2 Add `copyAnchorLinkOnly()`: build the same `paperland://paper/<id>?h=<hash>&s=<start>&e=<end>` URL, write `![#](<url>)` to the clipboard, show a distinct toast ("已复制锚点链接"), clear the selection, and close popups — mirroring the existing handler's guards (`pendingSelection`/`contentHash`/`paperId`) and teardown.
...
- [x] 3.1 In the running app, select text in a Q&A answer / note preview: confirm the `Copy` button copies `<markdown> [#](paperland://…)` (unchanged from before) and the `Link2` button copies exactly `![#](paperland://…)` with no content.
- [x] 3.2 Confirm both buttons are hidden for anonymous users and that each shows its own toast.

## 4. Docs & spec sync

- [x] 4.1 Update `docs/frontend-architecture.md` highlight-toolbar description ("浮动工具栏（4 色 + 复制为锚点链接）") to reflect the two copy actions (content+link with the copy icon, link-only `![#]` with the link icon).
