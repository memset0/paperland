...
- **THEN** the response SHALL include only that user's notes with non-empty bodies across papers, each annotated with `paper_id` and `paper_title`

#### Scenario: Empty root notes are excluded
- **WHEN** a user has papers whose only note is an empty root note
- **THEN** those empty root notes SHALL NOT appear in the `GET /api/notes` response
