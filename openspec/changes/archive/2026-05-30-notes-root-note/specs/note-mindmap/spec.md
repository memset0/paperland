...
The notes of a (user, paper) SHALL be presented as a branching mind-map rooted at the root note — an automatically laid-out hierarchical node view where parent/child relationships are shown as connected nodes (not a left-indented outline, and not a free-placement infinite canvas). Each node SHALL display only the note's title (the root node SHALL display a root label).

#### Scenario: Tree renders as a branching map
- **WHEN** an authenticated user views a paper's notes
- **THEN** the notes SHALL render as a branching mind-map rooted at the root note with parent/child nodes connected, each node showing only its title

#### Scenario: Node shows title only
- **WHEN** a node is displayed in the mind-map
- **THEN** it SHALL show the note's title (or a root label for the root node) and SHALL NOT inline the full body
