# notes-code-editor Specification

## Purpose

Provide a single, shared Monaco-based Markdown editing surface for all note editing in the app — both the floating editor windows (section and whole-document) and the left-panel edit/split editor — replacing the plain `<textarea>`. The editor highlights Markdown and LaTeX math, follows the app's light/dark theme live, is tuned for prose (soft-wrap, line numbers, no minimap or IntelliSense), preserves every existing note-editing behavior (autosave/commit, IME guarding, paste-to-upload, heading demotion, conflict detection), and is loaded on demand so pages without note editing never download it.
