...
#### Scenario: Walkthrough uses the same window
- **WHEN** a user opens the walkthrough for editing
- **THEN** it SHALL open in the same floating-window editor used by small notes

### Requirement: Window title shows the edited content
The window SHALL display, at its top, the title of the content being edited (the small note's title, or a walkthrough label).
