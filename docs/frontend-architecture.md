...
| **模板来源** | `templates/` 目录下的 .md 文件 | 用户输入 |
...
- 存放在 `templates/` 目录下
- 每个模板为一个 `.md` 文件，文件名即为模板名
- 示例：`templates/abstract.md`、`templates/method.md`、`templates/experiment.md`
...
- 手动点击 [重新生成] 按钮才会触发
- 重新从 `templates/` 目录读取最新模板内容（模板可能已更新）
...
alphaxiv_service:  (暂不开发)
  depends_on: [arxiv_id]
  produces:   [contents.alphaxiv]
```
...
corpus_id ──→ semantic_scholar_service ──→ arxiv_id ──┬──→ arxiv_service ──→ pdf_path ──→ pdf_parse_service ──→ contents.pdf_parsed
                                                      │
                                                      └──→ alphaxiv_service ──→ contents.alphaxiv
```
